...
The live origin is `https://playvejora.dpdns.org`. Confirm a release with `/version.info`. The workers.dev URL still works as a fallback.
...
`dpdns.org` is on the public suffix list, so `playvejora.dpdns.org` is a registrable domain. DigitalPlat only stores the nameserver hostnames; Cloudflare hosts the zone.
...
Those two values go in DigitalPlat as NS1 and NS2. The zone is already `active`, and the Worker is attached as a custom domain, so `SITE_URL` in `wrangler.jsonc` is `https://playvejora.dpdns.org`.
